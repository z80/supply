...
void setPwm( uint32_t mask, int value )
{
	...
	if ( mask & (1<<1) )
		pwmEnableChannel( &PWMD3, 0, PWM_PERCENTAGE_TO_WIDTH( &PWMD3, value ) );
	if ( mask & (1<<2) )
		pwmEnableChannel( &PWMD3, 1, PWM_PERCENTAGE_TO_WIDTH( &PWMD3, value ) );
	if ( mask & (1<<3) )
		pwmEnableChannel( &PWMD4, 2, PWM_PERCENTAGE_TO_WIDTH( &PWMD4, value ) );
	if ( mask & (1<<4) )
	...
	if ( mask & (1<<7) )
		pwmEnableChannel( &PWMD2, 0, PWM_PERCENTAGE_TO_WIDTH( &PWMD2, value ) );
}
